...
1. Uses `rayon` to scan folders and process images **parallelized**.
2. Generates color palettes using **K-Means with weighted centroid placement** to capture both dominant and visually important accent colors.
3. The app maintains a broad internal palette for precise matching while generating a curated, aesthetic palette for the UI.
4. Thumbnails and analysis results are stored in a local JSON cache, ensuring that subsequent launches are near-instant.
